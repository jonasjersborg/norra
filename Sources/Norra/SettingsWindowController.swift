...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private func aboutPane() -> NSView {
        ...
        let name = NSTextField(labelWithString: "Norra")
    ...
    @objc private func removeCarAction() {
        ...
        let alert = NSAlert()
        alert.messageText = last ? L("Sign out of Norra?") : L("Sign out and forget this car?")
        alert.informativeText = last
            ? String(format: L("Norra will forget the login for %@ and stop reading your car."),
                     Preferences.email)
            : String(format: L("Norra will forget the login for %@."), Preferences.email)
