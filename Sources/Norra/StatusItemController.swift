...
final class StatusItemController {
    ...
    init(onRefresh: @escaping () -> Void, onSettings: @escaping () -> Void) {
        ...
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "bolt.car", accessibilityDescription: "Norra")
    ...
    func render(data: CarData?, error: String?, authenticated: Bool) {
        ...
        statusItem.button?.image = NSImage(systemSymbolName: symbol, accessibilityDescription: "Norra")
    ...
    private func buildMenu(data: CarData?, error: String?) -> NSMenu {
        ...
        if let data {
            ...
            // `defaults write com.weareheavy.norra debug_charging_type -string DC`
    ...
    // The formatting itself lives in NorraShared so the widget renders the
